...
(() => {
  ...
  const ICONS = {
    ...
    espressoCold: `
      <svg viewBox="0 0 392.656 392.656" fill="currentColor" stroke="currentColor"
  ...
  function buildCategoryButtons() {
    CATEGORIES.forEach((category) => {
      ...
      textWrap.className = "category-btn__text";

      const title = document.createElement("span");
      title.className = "category-btn__title";
      title.textContent = category.label;

      const example = document.createElement("span");
      example.className = "category-btn__example";
      example.textContent = categoryExample(category.id);

      textWrap.append(title, example);

      const iconWrap = document.createElement("span");
      iconWrap.className = "category-btn__icon";
      iconWrap.innerHTML = ICONS[category.icon];

      button.append(textWrap, iconWrap);
